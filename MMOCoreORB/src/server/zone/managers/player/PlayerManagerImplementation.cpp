...
#include "server/zone/objects/scene/events/DestroyObjectTask.h"
#include "server/zone/objects/tangible/events/UpdateContainerPermissionsTask.h"
...
void PlayerManagerImplementation::killPlayer(TangibleObject* attacker, CreatureObject* player, int typeofdeath, bool isCombatAction) {
	...
	//handle hardcore death bit
	ManagedReference<TangibleObject*> corpse = player->getZoneServer()->createObject(STRING_HASHCODE("object/tangible/container/drum/treasure_drum.iff"), 0).castTo<TangibleObject*>();
	

	if (corpse != nullptr) {

		Locker crossLock(corpse, player);
		
		//set corpse name to fit dead player
		corpse->setCustomObjectName(player->getFirstName() + " " + player->getLastName() + "'s stuff", true);

		corpse->setContainerDefaultAllowPermission(ContainerPermissions::OPEN);

		//destroy corpse after 24 hours
		Reference<DestroyObjectTask*> destroyTask = new DestroyObjectTask(corpse);
        corpse->addPendingTask("destroy", destroyTask, 24 * 60 * 60 * 1000);

		//handle corpse positioning and add to world
		corpse->initializePosition(player->getPositionX(), player->getPositionZ(), player->getPositionY());
		corpse->setDirection(player->getDirectionW(), player->getDirectionX(), player->getDirectionY(), player->getDirectionZ());
		ManagedReference<SceneObject*> parent = player->getParent().get();
		if (parent != nullptr && parent->isCellObject())
			parent->transferObject(corpse, -1);
		else
			player->getZone()->transferObject(corpse, -1, true);

		//transfer inventory items to corpse
		ManagedReference<SceneObject*> playerInventory = player->getSlottedObject("inventory");
		if (playerInventory != nullptr) {
			int totalItems = playerInventory->getContainerObjectsSize();
			if (totalItems > 0) {
				for (int i = totalItems - 1; i >= 0; --i) {
					ManagedReference<SceneObject*> object = playerInventory->getContainerObject(i);
					if (object == nullptr) 
						continue;
					player->getZoneServer()->getObjectController()->transferObject(object, corpse, -1, true, true);
				}
				
			}
		}

		//transfer slotted objects to corpse
		//would leave some stuff behind, so looping till it's all gone
		int loop = 0;
		while(player->getSlottedObjectsSize() > 7) {
			//to stop it, if it keeps going for some reason
			loop++;
			if (loop > 10)
				break;

			for (int i = 0; i < player->getSlottedObjectsSize(); ++i) {
				ManagedReference<SceneObject*> item = player->getSlottedObject(i);
				bool transfer = true;
				if (item != nullptr) {
					for (int k = 0; k < item->getArrangementDescriptorSize(); ++k) {
						const Vector<String>* descriptors = item->getArrangementDescriptor(k);
					...
					if (transfer == true) {
						player->getZoneServer()->getObjectController()->transferObject(item, corpse, -1, true, true);
					}
				}
			}
		}

		//remove and destroy vehicles, pets and droids
		ManagedReference<SceneObject*> playerDatapad = player->getSlottedObject("datapad");
		if (playerDatapad != nullptr) {
			int datapadItems = playerDatapad->getContainerObjectsSize();
			if (datapadItems > 0) {
				for (int i = datapadItems - 1; i >= 0; --i) {
					SceneObject* sceneO = playerDatapad->getContainerObject(i);
					if (sceneO == nullptr) 
						continue;

					Locker locki(sceneO);

					if (sceneO->isVehicleControlDevice()) {
							ManagedReference<VehicleControlDevice*> vehicleControlDevice = cast<VehicleControlDevice*>(sceneO);
							if (vehicleControlDevice != nullptr) {
								vehicleControlDevice->storeObject(player, true);
							}
					}
					else if (sceneO->isPetControlDevice()) {
							ManagedReference<PetControlDevice*> petControlDevice = cast<PetControlDevice*>(sceneO);
		...
		if (attacker->isPlayerCreature() || attacker->isPet()) {
			...
			ManagedReference<CreatureObject*> attackerCreo = attacker->asCreatureObject();

			if (attackerCreo != nullptr) {
				if (attackerCreo->isPet()) {
					ManagedReference<CreatureObject*> petMaster = attackerCreo->getLinkedCreature().get();
					if (petMaster != nullptr) {
						if (petMaster->isGrouped()) {
							ownerID = petMaster->getGroupID();
						}
						else {
							ownerID = petMaster->getObjectID();
						}
					}	
				}
				else {
					if (attackerCreo->isGrouped()) {
						ownerID = attackerCreo->getGroupID();
					}
					else {
						ownerID = attackerCreo->getObjectID();
					}
				}

				//change container permissions after some time
				Reference<UpdateContainerPermissionsTask*> updatePerms = new UpdateContainerPermissionsTask(corpse, attackerCreo, true);
       			corpse->addPendingTask("updateContainerPerms", updatePerms, 2 * 60 * 1000);
			}
		}

		corpse->setContainerOwnerID(ownerID);

		//handle credit chip
		// ManagedReference<TangibleObject*> creditChip = player->getZoneServer()->createObject(STRING_HASHCODE("object/tangible/item/loot_credit_chip.iff"), 1).castTo<TangibleObject*>();
		// if (creditChip != nullptr) {
		// 	player->getZoneServer()->getObjectController()->transferObject(creditChip, corpse, -1, true, true);
		// }

	}

	//remove all cash credits and add to killer -- leave for now instead of creating a chip
	int playerCredits = player->getCashCredits();
	player->subtractCashCredits(playerCredits);
	if (attacker->isPlayerCreature() || attacker->isPet()) {
		ManagedReference<CreatureObject*> attackerCreature = attacker->asCreatureObject();
		if (attackerCreature != nullptr) {
			if (attacker->isPet()) {
				ManagedReference<CreatureObject*> petMaster = attackerCreature->getLinkedCreature().get();
				if (petMaster != nullptr) {
					petMaster->addCashCredits(playerCredits);
				}	
			}
			else {
				attackerCreature->addCashCredits(playerCredits);
			}
		}
	}

	//end hardcore death bit
...
void PlayerManagerImplementation::sendPlayerToCloner(CreatureObject* player, uint64 clonerID, int typeofdeath) {
	...
	//start hardcore clone part
	
	//grant 3 hour xp buff based on skillpoints spent (and jedi skill points) 1 % per skillpt and 2 additional % per jedi skillpt
	if (!player->hasBuff(STRING_HASHCODE("clone_xp_buff") && player->checkCooldownRecovery("xp_buff_on_clone"))) {
		int spentSkillPoints = 250 - ghost->getSkillPoints();

		int jediSkillPoints = ghost->getSpentJediSkillPoints();

		int totalXpIncrease = spentSkillPoints + jediSkillPoints * 2;

		if (totalXpIncrease > 29) {
			ManagedReference<Buff*> cloneXpBuff = new Buff(player, STRING_HASHCODE("clone_xp_buff"), 3 * 60 * 60 * 1000, BuffType::SKILL);
			if (cloneXpBuff != nullptr) {
				Locker locker(cloneXpBuff);
				cloneXpBuff->setSkillModifier("xp_increase", totalXpIncrease);
				player->addBuff(cloneXpBuff);
				player->addCooldown("xp_buff_on_clone", 60 * 60 * 1000);
				locker.release();
			}
		}
	}
