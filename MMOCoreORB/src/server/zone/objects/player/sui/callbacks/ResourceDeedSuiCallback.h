	...
	void run(CreatureObject* creature, SuiBox* sui, uint32 eventIndex, Vector<UnicodeString>* args) {
		...
		if (backPressed) {
			...
			if (spawn != nullptr) {
				...
				int resAmount = deed->getAmount();
				// String deedName = deed->getDisplayedName();

				// if (deedName.beginsWith("Tiny")) {
				// 	resAmount = 50;
				// }
				// else if (deedName.beginsWith("Small")) {
				// 	resAmount = 100;
				// }
				// else if (deedName.beginsWith("Regular")) {
				// 	resAmount = 500;
				// }
				// else if (deedName.beginsWith("Big")) {
				// 	resAmount = 1000;
				// }
				// else if (deedName.beginsWith("Small")) {
				// 	resAmount = 5000;
				...
				resourceManager->givePlayerResource(creature, nodeName, resAmount);
