...
function JunkDealerConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	if screenID == "ask_for_loot" then
		...
		--smuggler goods
		clonedScreen:addOption(self.smugglerGoodsText, "smuggler_goods")
	...
	elseif string.find(screenID, "offer_smuggler_goods_random") ~= nil then
		local pInventory = SceneObject(pPlayer):getSlottedObject("inventory")
		if pInventory == nil or SceneObject(pInventory):isContainerFullRecursive() then
			clonedScreen:setDialogTextStringId("@conversation/junk_dealer_generic:smuggler_goods_no_space") -- I am sorry but you do not have any space in your inventory for me to give you a kit. Maybe you should clear out some room and talk to me again if you are still interested.
		else
			local credits = CreatureObject(pPlayer):getCashCredits()

			if (credits < 25000) then
				clonedScreen:setDialogTextStringId("@conversation/junk_dealer_generic:smuggler_goods_no_credits")
			else
				CreatureObject(pPlayer):subtractCashCredits(25000)
				giveItem(pInventory, "object/tangible/container/loot/smuggler_goods_random.iff", -1)
			end
		end
	end
...
JunkDealerGenericConvoHandler = JunkDealerConvoHandler:new {
	...
	smugglerGoodsText = "conversation/junk_dealer_generic:smuggler_goods_1"
}
